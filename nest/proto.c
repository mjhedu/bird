...
static char *p_states[] =
  { "DOWN", "START", "UP", "STOP" };
static char *c_states[] =
  { "HUNGRY", "???", "HAPPY", "FLUSHING" };

static void
proto_flush_loop (void *);
static void
proto_shutdown_loop (struct timer *);
static void
...
static int
proto_reconfigure (struct proto *p, struct proto_config *oc,
		   struct proto_config *nc, int type)
{
  ...
  if ((nc->protocol != oc->protocol) || (nc->disabled != p->disabled)
...
proto_rethink_goal (struct proto *p)
{
  ...
  if (p->reconfiguring && p->core_state == FS_HUNGRY
...
static void
graceful_restart_done (struct timer *t);
...
graceful_restart_show_status (void)
{
  ...
  cli_msg(-24, "  Wait timer is %d/%d", tm_remains (gr_wait_timer),
	  config->gr_wait);
}
...
{
  const char *actions[] =
    { [PLA_WARN] = "warn", [PLA_BLOCK] = "block", [PLA_RESTART] = "restart",
	[PLA_DISABLE] = "disable", };
 ...
 */
void
proto_notify_limit (struct announce_hook *ah, struct proto_limit *l, int dir,
		    u32 rt_count)
{
  const char *dir_name[PLD_MAX] =
    { "receive", "import", "export" };
  const byte dir_down[PLD_MAX] =
    { PDC_RX_LIMIT_HIT, PDC_IN_LIMIT_HIT, PDC_OUT_LIMIT_HIT };
  ...
  if (!l->state || ((l->action == PLA_WARN) && (rt_count == l->limit)))
    log (L_WARN "Protocol %s hits route %s limit (%d), action: %s", p->name,
	 dir_name[dir], l->limit, proto_limit_name (l));
...
proto_fell_down (struct proto *p)
{
  ...
  if (all_routes != 0)
    log (L_ERR "Protocol %s is down but still has %d routes", p->name,
	 all_routes);
...
#define P(x,y) ((x << 4) | y)
  switch (P(p->proto_state, p->core_state))
    {
    case P(PS_DOWN, FS_HUNGRY):
      return "down";
    case P(PS_START, FS_HUNGRY):
    case P(PS_START, FS_HAPPY):
      return "start";
...
proto_show_stats (struct proto_stats *s, int in_keep_filtered)
{
  if (in_keep_filtered)
    cli_msg(
	-1006,
	"  Routes:         %u imported, %u filtered, %u exported, %u preferred",
...
proto_show_basic_info (struct proto *p)
{
  ...
  if (graceful_restart_state == GRS_ACTIVE)
    cli_msg(-1006, "  GR recovery:   %s%s", p->gr_lock ? " pending" : "",
	    p->gr_wait ? " waiting" : "");
...
proto_cmd_show (struct proto *p, unsigned int verbose, int cnt)
{
  ...
  cli_msg(-1002, "%-8s %-8s %-8s %-5s  %-10s  %s", p->name, p->proto->name,
	  p->table->name, proto_state_name (p), tbuf, buf);
  if (verbose)
    {
      if (p->cf->dsc)
...
static void
proto_apply_cmd_symbol (struct symbol *s, void
(*cmd) (struct proto *, unsigned int, int),
			unsigned int arg)
{
...
static void
proto_apply_cmd_patt (char *patt, void
(*cmd) (struct proto *, unsigned int, int),
		      unsigned int arg)
{
  int cnt = 0;
...
void
proto_apply_cmd (struct proto_spec ps, void
(*cmd) (struct proto *, unsigned int, int),
