...
static byte bgp_mandatory_attrs[] =
  { BA_ORIGIN, BA_AS_PATH
#ifndef IPV6
      , BA_NEXT_HOP
#endif
    };
...
struct attr_desc
{
  ...
  int allow_in_ebgp;
  int
  (*validate) (struct bgp_proto *p, byte *attr, int len);
  void
  (*format) (eattr *ea, byte *buf, int buflen);
};
...
static int
validate_path (struct bgp_proto *p, int as_path, int bs, byte *idata,
  ...
  while (len)
	...
	case AS_PATH_CONFED_SET:
	  if (as_path && path_segment_contains (a, bs, p->remote_as))
	    {
	      log (
		  L_WARN "%s: AS_CONFED_* segment with peer ASN found, misconfigured confederation?",
		  p->p.name);
	      return -1;
	    }

	  log (
	      L_WARN "%s: %s_PATH attribute contains AS_CONFED_* segment, skipping segment",
...
static int
bgp_check_reach_nlri (struct bgp_proto *p UNUSED, byte *a UNUSED,
		      int len UNUSED)
{
#ifdef IPV6
  p->mp_reach_start = a;
...
static int
bgp_check_unreach_nlri (struct bgp_proto *p UNUSED, byte *a UNUSED,
			int len UNUSED)
{
#ifdef IPV6
...
void
bgp_attach_attr (ea_list **to, struct linpool *pool, unsigned attr,
		 uintptr_t val)
{
...
byte *
bgp_attach_attr_wa (ea_list **to, struct linpool *pool, unsigned attr,
		    unsigned len)
{
  struct adata *ad = bgp_alloc_adata (pool, len);
...
bgp_encode_attrs (struct bgp_proto *p, byte *w, ea_list *attrs, int remains)
{
  ...
  for (i = 0; i < attrs->count; i++)
      ...
      if ((code == BA_AS_PATH) && (!p->as4_session))
	{
	  ...
	  rv = bgp_encode_attr_hdr (w, BAF_OPTIONAL | BAF_TRANSITIVE,
	  BA_AS4_PATH,
				    len);
	  ADVANCE(w, remains, rv);
      ...
      if ((code == BA_AGGREGATOR) && (!p->as4_session))
	{
      ...
      /* Skip empty sets */
      if (((type == EAF_TYPE_INT_SET) || (type == EAF_TYPE_EC_SET))
	  && (len == 0))
	continue;
...
bgp_rehash_buckets (struct bgp_proto *p)
{
  ...
  if (p->hash_limit >= 65536)
    p->hash_limit = ~0;
  new = p->bucket_hash = mb_allocz (p->p.pool,
				    p->hash_size * sizeof(struct bgp_bucket *));
  mask = p->hash_size - 1;
  for (i = 0; i < oldn; i++)
    while (b = old[i])
      {
...
bgp_new_bucket (struct bgp_proto *p, ea_list *new, unsigned hash)
{
  ...
  for (i = 0; i < new->count; i++)
	...
	size += BIRD_ALIGN(sizeof(struct adata) + a->u.ptr->length,
			   CPU_STRUCT_ALIGN);
    }
  ...
  for (i = 0; i < new->count; i++)
      ...
      if (!(a->type & EAF_EMBEDDED))
	{
	  ...
	  a->u.ptr = na;
	  dest += BIRD_ALIGN(sizeof(struct adata) + na->length,
			     CPU_STRUCT_ALIGN);
	}
    }
...
bgp_get_bucket (struct bgp_proto *p, net *n, ea_list *attrs, int originate)
{
  ...
  for (i = 0; i < cnt; i++)
      ...
      if (ATTR_KNOWN(code))
	{
	  ...
	  a->flags = (a->flags & BAF_PARTIAL)
	      | bgp_attr_table[code].expected_flags;
	  if (code < 32)
	    seen |= 1 << code;
	}
      else
	{
      ...
      *d = *a;
      if ((d->type & EAF_ORIGINATED) && !originate
	  && (d->flags & BAF_TRANSITIVE) && (d->flags & BAF_OPTIONAL))
	d->flags |= BAF_PARTIAL;
	...
	case EAF_TYPE_INT_SET:
	  {
	    ...
	    bgp_normalize_int_set ((u32 *) z->data, (u32 *) d->u.ptr->data,
				   z->length / 4);
	    d->u.ptr = z;
	    break;
	  }
  ...
  for (i = 0; i < ARRAY_SIZE(bgp_mandatory_attrs); i++)
    if (!(seen & (1 << bgp_mandatory_attrs[i])))
      {
	log (L_ERR "%s: Mandatory attribute %s missing in route %I/%d",
	     p->p.name, bgp_attr_table[bgp_mandatory_attrs[i]].name,
  ...
  if (!a || ipa_equal(p->cf->remote_ip, *(ip_addr * )a->u.ptr->data))
    {
      log (L_ERR "%s: Invalid NEXT_HOP attribute in route %I/%d", p->p.name,
	   n->n.prefix, n->n.pxlen);
      return NULL;
    }
...
{
  struct bgp_prefix *bp = HASH_FIND(p->prefix_hash, PXH, prefix, pxlen,
				    path_id);
...
void
bgp_rt_notify (struct proto *P, rtable *tbl UNUSED, net *n, rte *new,
	       rte *old UNUSED, ea_list *attrs)
{
  ...
  if (new)
	...
	{
	  buck = p->withdraw_bucket = mb_alloc (P->pool,
...
static int
bgp_create_attrs (struct bgp_proto *p, rte *e, ea_list **attrs,
		  struct linpool *pool)
{
  ...
  *attrs = ea;
  ...
  bgp_set_attr (
      ea->attrs,
      BA_ORIGIN,
      ((rta->source == RTS_OSPF_EXT1) || (rta->source == RTS_OSPF_EXT2)) ?
  ...
  if (p->cf->next_hop_self || rta->dest != RTD_ROUTER
      || ipa_equal(rta->gw, IPA_NONE) || ipa_is_link_local(rta->gw)
...
bgp_path_prepend (rte *e, ea_list **attrs, struct linpool *pool, u32 as)
{
  ...
  bgp_attach_attr (attrs, pool, BA_AS_PATH,
		   (uintptr_t) as_path_prepend (pool, a->u.ptr, as));
}

static inline void
bgp_cluster_list_prepend (rte *e, ea_list **attrs, struct linpool *pool,
			  u32 cid)
{
  eattr *a = ea_find (e->attrs->eattrs, EA_CODE(EAP_BGP, BA_CLUSTER_LIST));
  bgp_attach_attr (attrs, pool, BA_CLUSTER_LIST,
		   (uintptr_t) int_set_add (pool, a ? a->u.ptr : NULL, cid));
}

static int
bgp_update_attrs (struct bgp_proto *p, rte *e, ea_list **attrs,
		  struct linpool *pool, int rr)
{
  eattr *a;
  ...
  if (rr)
	...
	bgp_attach_attr (attrs, pool, BA_ORIGINATOR_ID, src->remote_id);
      ...
      bgp_cluster_list_prepend (
	  e, attrs, pool,
	  src->rr_client ? src->rr_cluster_id : p->rr_cluster_id);

      /* Two RR clients with different cluster ID, hmmm */
      if (src->rr_client && p->rr_client
...
int
bgp_import_control (struct proto *P, rte **new, ea_list **attrs,
		    struct linpool *pool)
{
  rte *e = *new;
  struct bgp_proto *p = (struct bgp_proto *) P;
  struct bgp_proto *new_bgp =
      (e->attrs->src->proto->proto == &proto_bgp) ?
  ...
  if (new_bgp)
	...
	return -1;
      ...
      if (p->local_as == new_bgp->local_as && p->is_internal
	  && new_bgp->is_internal)
	{
...
use_deterministic_med (rte *r)
{
  ...
  return (P->proto == &proto_bgp)
      && ((struct bgp_proto *) P)->cf->deterministic_med;
}
 ...
 */
static struct adata *
bgp_merge_as_paths (struct adata *old2, struct adata *old4, int req_as,
		    struct linpool *pool)
{
  ...
  if (old4)
    memcpy (newa->data + ol, old4->data, old4->length);
...
bgp_reconstruct_4b_atts (struct bgp_proto *p, rta *a, struct linpool *pool)
{
  ...
  if (a2)
	  ...
	  if (a2_as != AS_TRANS)
	    {
	      ...
	      p2->u.ptr = bgp_merge_as_paths (p2->u.ptr, NULL, AS_PATH_MAXLEN,
					      pool);
      ...
      else
	{
	  ...
	  if ((a2_as == AS_TRANS) && !a4_removed)
	    log (
		L_WARN "%s: AGGREGATOR attribute contain AS_TRANS, but AS4_AGGREGATOR is missing",
		p->p.name);
	}
    }
  else if (a4)
    log (
	L_WARN "%s: AS4_AGGREGATOR attribute received, but AGGREGATOR attribute is missing",
	p->p.name);
  ...
  if (p4 && (p4_len < 0))
    log (L_WARN "%s: AS4_PATH attribute is malformed, skipping attribute",
	 p->p.name);

  if ((p4_len <= 0) || (p2_len < p4_len))
    p2->u.ptr = bgp_merge_as_paths (p2->u.ptr, NULL, AS_PATH_MAXLEN, pool);
  else
    p2->u.ptr = bgp_merge_as_paths (p2->u.ptr, p4->u.ptr, p2_len - p4_len,
				    pool);
}
		  ...
		  struct linpool *pool, int mandatory)
{
  ...
  while (len)
      ...
      if (ATTR_KNOWN(code))
	{
	  ...
	  if (desc->validate)
		...
		{
		  log (
		  L_WARN "%s: Attribute %s is malformed, withdrawing update",
  ...
  if (bgp_originator_id_loopy (bgp, a) || bgp_cluster_list_loopy (bgp, a))
    goto withdraw;

  /* If there's no local preference, define one */
  if (!(seen[0] & (1 << BA_LOCAL_PREF)))
    bgp_attach_attr (&a->eattrs, pool, BA_LOCAL_PREF,
		     bgp->cf->default_local_pref);

  return a;

  withdraw: return NULL;

  malformed: bgp_error (conn, 3, 1, NULL, 0);
  return NULL;

  err: bgp_error (conn, 3, errcode, attr_start, z + l - attr_start);
  return NULL;
}
...
bgp_init_bucket_table (struct bgp_proto *p)
{
  ...
  p->bucket_hash = mb_allocz (p->p.pool,
			      p->hash_size * sizeof(struct bgp_bucket *));
