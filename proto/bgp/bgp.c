...
bgp_startup(struct bgp_proto *p)
{
  ...
  bgp_hook_run (BGP_HOOK_START, p, NULL);
}
...
bgp_down(struct bgp_proto *p)
{
  ...
  bgp_hook_run (BGP_HOOK_DOWN, p, NULL);
}
...
bgp_conn_set_state(struct bgp_conn *conn, unsigned new_state)
{
  ...
  bgp_hook_run(BGP_HOOK_CHANGE_STATE, conn->bgp, NULL);
}
...
bgp_conn_enter_openconfirm_state(struct bgp_conn *conn)
{
  ...
  if (bgp_hook_run (BGP_HOOK_ENTER_OPENCONFIRM, conn->bgp, NULL) & HOOK_STATUS_BAD)
     bgp_stop(conn->bgp, 0);
}
...
bgp_conn_enter_established_state(struct bgp_conn *conn)
{
  ...
  if (bgp_hook_run (BGP_HOOK_ENTER_ESTABLISHED, p, NULL) & HOOK_STATUS_BAD)
    bgp_stop(p, 0);
...
bgp_conn_leave_established_state(struct bgp_proto *p)
{
  ...
  bgp_hook_run(BGP_HOOK_LEAVE_ESTABLISHED, p, NULL);
...
bgp_conn_enter_close_state(struct bgp_conn *conn)
{
  ...
  bgp_hook_run(BGP_HOOK_ENTER_CLOSE, p, NULL);
}
...
bgp_conn_enter_idle_state(struct bgp_conn *conn)
{
  ...
  bgp_hook_run(BGP_HOOK_ENTER_IDLE, p, NULL);
}
...
bgp_handle_graceful_restart(struct bgp_proto *p)
{
  ...
  bgp_hook_run(BGP_HOOK_NEIGH_GRESTART, p, NULL);
...
bgp_refresh_begin(struct bgp_proto *p)
{
  ...
  bgp_hook_run(BGP_HOOK_REFRESH_BEGIN, p, NULL);
}
...
bgp_refresh_end(struct bgp_proto *p)
{
  ...
  bgp_hook_run(BGP_HOOK_REFRESH_END, p, NULL);
}
...
bgp_connected(sock *sk)
{
  ...
  if (bgp_hook_run (BGP_HOOK_CONN_OUTBOUND, p, NULL) & HOOK_STATUS_BAD)
    bgp_stop(conn->bgp, 0);
}
...
bgp_connect_timeout(timer *t)
{
  ...
  bgp_hook_run (BGP_HOOK_CONN_TIMEOUT, p, NULL);
...
bgp_incoming_connection(sock *sk, int dummy UNUSED)
{
  ...
  if ( bgp_hook_run(BGP_HOOK_CONN_INBOUND, p, NULL) & HOOK_STATUS_BAD )
    {
...
bgp_start_neighbor(struct bgp_proto *p)
{
  ...
  if ( bgp_hook_run(BGP_HOOK_NEIGH_START, p, NULL) != 2 )
    bgp_initiate(p);
}
...
bgp_feed_begin(struct proto *P, int initial)
{
  ...
  if (!initial && p->cf->enable_refresh &&
	...
	return;
      ...
      bgp_hook_run(BGP_HOOK_FEED_BEGIN, p, NULL);
    }
}
...
bgp_feed_end(struct proto *P)
{
  ...
  bgp_hook_run(BGP_HOOK_FEED_END, p, NULL);
}
...
bgp_shutdown(struct proto *P)
{
  ...
  bgp_hook_run (BGP_HOOK_SHUTDOWN, p, NULL);
...
bgp_init(struct proto_config *C)
{
  ...
  bgp_hook_run(BGP_HOOK_INIT, p, NULL);
...
bgp_reconfigure(struct proto *P, struct proto_config *C)
{
  ...
  if (bgp_hook_run (BGP_HOOK_RECONFIGURE, p, NULL) & HOOK_STATUS_BAD)
      bgp_stop(p, 0);
