...
static void
bgp_connect (struct bgp_proto *p);
static void
...
bgp_open (struct bgp_proto *p)
{
  ...
  if (!bgp_listen_sk)
    bgp_listen_sk = bgp_setup_listen_sk (cfg->listen_bgp_addr,
					 cfg->listen_bgp_port,
					 cfg->listen_bgp_flags);
  ...
  if (p->cf->password)
    if (sk_set_md5_auth (bgp_listen_sk, p->cf->remote_ip, p->cf->iface,
			 p->cf->password) < 0)
      {
  ...
  if (p->startup_delay)
    {
      ...
      BGP_TRACE(D_EVENTS, "Startup delayed by %d seconds due to errors",
		p->startup_delay);
...
bgp_close (struct bgp_proto *p, int apply_md5)
{
  ...
  if (p->cf->password && apply_md5)
    if (sk_set_md5_auth (bgp_listen_sk, p->cf->remote_ip, p->cf->iface, NULL)
	< 0)
      sk_log_error (bgp_listen_sk, p->p.name);
...
bgp_update_startup_delay (struct bgp_proto *p)
{
  ...
  if (p->last_proto_error
      && ((now - p->last_proto_error) >= (int) cf->error_amnesia_time))
    p->startup_delay = 0;
...
bgp_decision (void *vp)
{
  ...
  if ((p->p.proto_state == PS_START) && (p->outgoing_conn.state == BS_IDLE)
      && (p->incoming_conn.state != BS_OPENCONFIRM) && (!p->cf->passive))
    bgp_active (p);

  if ((p->p.proto_state == PS_STOP) && (p->outgoing_conn.state == BS_IDLE)
...
bgp_conn_enter_openconfirm_state (struct bgp_conn *conn)
{
  ...
  if (bgp_hook_run (BGP_HOOK_ENTER_OPENCONFIRM, conn->bgp, NULL,
  NULL) & HOOK_STATUS_BAD)
    bgp_stop (conn->bgp, 0);
}
...
bgp_conn_enter_established_state (struct bgp_conn *conn)
{
  ...
  int peer_gr_ready = conn->peer_gr_aware
      && !(conn->peer_gr_flags & BGP_GRF_RESTART);
...
bgp_refresh_begin (struct bgp_proto *p)
{
  if (p->load_state == BFS_LOADING)
    {
      log (L_WARN "%s: BEGIN-OF-RR received before END-OF-RIB, ignoring",
	   p->p.name);
      return;
    }
...
bgp_refresh_end (struct bgp_proto *p)
{
  if (p->load_state != BFS_REFRESHING)
    {
      log (L_WARN "%s: END-OF-RR received without prior BEGIN-OF-RR, ignoring",
	   p->p.name);
      return;
    }
...
bgp_connect (struct bgp_proto *p) /* Enter Connect state and start establishing connection */
{
  ...
  BGP_TRACE(D_EVENTS, "Connecting to %I%J from local address %I%J", s->daddr,
	    p->cf->iface, s->saddr,
	    ipa_is_link_local(s->saddr) ? s->iface : NULL);
  bgp_setup_conn (p, conn);
  bgp_setup_sk (conn, s);
...
bgp_incoming_connection (sock *sk, int dummy UNUSED)
{
  ...
  if (!p)
    {
      log (L_WARN "BGP: Unexpected connect from unknown address %I%J (port %d)",
	   ...
	   sk->dport);
      hook_run (HOOK_CONN_INBOUND_UNEXPECTED, config,
		bgp_handle_invalid_in_conn, (void*) sk);
      rfree (sk);
      return 0;
    }
  ...
  BGP_TRACE(D_EVENTS, "Incoming connection from %I%J (port %d) %s", sk->daddr,
	    ipa_is_link_local(sk->daddr) ? sk->iface : NULL, sk->dport,
  ...
  if (bgp_hook_run (BGP_HOOK_CONN_INBOUND, p, NULL, NULL) & HOOK_STATUS_BAD)
    {
      log (L_ERR "%s: hook: child process aborted inbound connection",
	   p->p.name);
      rfree (sk);
      return 0;
    }
...
bgp_update_bfd (struct bgp_proto *p, int use_bfd)
{
  if (use_bfd && !p->bfd_req)
    p->bfd_req = bfd_request_session (p->p.pool, p->cf->remote_ip,
				      p->source_addr,
...
bgp_start_locked (struct object_lock *lock)
{
  ...
  if (!n)
    {
      log (L_ERR "%s: Invalid remote address %I%J", p->p.name, cf->remote_ip,
	   cf->iface);
  ...
  if (n->scope <= 0)
    BGP_TRACE(D_EVENTS, "Waiting for %I%J to become my neighbor", cf->remote_ip,
	      cf->iface);
  else if (p->cf->check_link && !(n->iface->flags & IF_LINK_UP))
...
bgp_start (struct proto *P)
{
  ...
  if (p->rr_client)
    p->rr_cluster_id =
	p->cf->rr_cluster_id ? p->cf->rr_cluster_id : p->local_id;
...
bgp_shutdown (struct proto *P)
{
  ...
  switch (P->down_code)
    ...
    case PDC_OUT_LIMIT_HIT:
      ...
      limit: bgp_store_error (p, NULL, BE_AUTO_DOWN, BEA_ROUTE_LIMIT_EXCEEDED);
      if (proto_restart)
	bgp_update_startup_delay (p);
      else
	p->startup_delay = 0;
...
bgp_check_config (struct bgp_config *c)
{
  ...
  if (c->c.in_limit && (c->c.in_limit->action == PLA_RESTART)
      && c->disable_after_error)
  ...
  if (c->multihop
      && (ipa_is_link_local(c->remote_ip) || ipa_is_link_local(c->source_addr)))
...
bgp_reconfigure (struct proto *P, struct proto_config *C)
{
  ...
  int same = !memcmp (
      ((byte *) old) + sizeof(struct proto_config),
 ...
 */
void
bgp_error (struct bgp_conn *c, unsigned code, unsigned subcode, byte *data,
	   int len)
{
  ...
  bgp_log_error (p, BE_BGP_TX, "Error", code, subcode, data,
		 (len > 0) ? len : -len);
...
static char *bgp_err_classes[] =
  { "", "Error: ", "Socket: ", "Received: ", "BGP Error: ",
      "Automatic shutdown: ", "" };
static char *bgp_misc_errors[] =
      { "", "Neighbor lost", "Invalid next hop", "Kernel MD5 auth failed",
	  "No listening socket", "Link down", "BFD session down",
	  "Graceful restart" };
static char *bgp_auto_errors[] =
...
bgp_last_errmsg (struct bgp_proto *p)
{
  switch (p->last_error_class)
    ...
    case BE_SOCKET:
      return
	  (p->last_error_code == 0) ?
	      "Connection closed" : strerror (p->last_error_code);
    case BE_BGP_RX:
    case BE_BGP_TX:
  ...
  if (P->proto_state == PS_START)
    {
      ...
      if ((p->start_state < BSS_CONNECT) && (p->startup_timer->expires))
	cli_msg(-1006, "    Error wait:       %d/%d",
		p->startup_timer->expires - now, p->startup_delay);

      if ((oc->state == BS_ACTIVE) && (oc->connect_retry_timer->expires))
	cli_msg(-1006, "    Connect delay:    %d/%d",
		oc->connect_retry_timer->expires - now,
		p->cf->connect_delay_time);
  ...
  else if (P->proto_state == PS_UP)
    {
      ...
      cli_msg(
	  -1006,
	  "    Neighbor caps:   %s%s%s%s%s%s",
	  c->peer_refresh_support ? " refresh" : "",
	  c->peer_enhanced_refresh_support ? " enhanced-refresh" : "",
	  c->peer_gr_able ?
	      " restart-able" : (c->peer_gr_aware ? " restart-aware" : ""),
	...
	cli_msg(-1006, "    Route limit:      %d/%d",
		p->p.stats.imp_routes + p->p.stats.filt_routes,
		P->cf->in_limit->limit);
      cli_msg(-1006, "    Hold timer:       %d/%d", tm_remains (c->hold_timer),
	      c->hold_time);
      cli_msg(-1006, "    Keepalive timer:  %d/%d",
	      tm_remains (c->keepalive_timer), c->keepalive_time);
    }

  if ((p->last_error_class != BE_NONE) && (p->last_error_class != BE_MAN_DOWN))
    {
      const char *err1 = bgp_err_classes[p->last_error_class];
      const char *err2 = bgp_last_errmsg (p);
...
struct protocol proto_bgp =
  { .name = "BGP", .template = "bgp%d", .attr_class = EAP_BGP, .preference =
  DEF_PREF_BGP, .config_size = sizeof(struct bgp_config), .init = bgp_init,
      .start = bgp_start, .shutdown = bgp_shutdown, .cleanup = bgp_cleanup,
      .reconfigure = bgp_reconfigure, .copy_config = bgp_copy_config,
      .get_status = bgp_get_status, .get_attr = bgp_get_attr, .get_route_info =
	  bgp_get_route_info, .show_proto_info = bgp_show_proto_info };
